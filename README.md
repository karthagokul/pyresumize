...
## Package Diagram
![Alt text](https://github.com/karthagokul/pyresumize/blob/main/package
...
Then do the below
    ...
    print(json)
s_pyresumize.png?raw=true  "Packages")
...
Currently the Engine supports PDF and DOCX File extensions , By implementing AbstractFileProcessor you can create your own parser to insert it to the engine via the method add_custom_text_processor. Here is an example snippet

    class MyfancyWordProcessor(AbstractFileProcessor):
        def process(self,file_input):
            # You can do all processing and return the text Data
            # if there is an issue in processing return None.

Now Let's connect the above processor to the Resume Parsing Engine like below to override the default DOCX Processor in the module

    process_engine=MyfancyWordProcessor()
    r_parser=ResumeEngine()
    r_parser.add_custom_text_processor(process_engine,"docx")
